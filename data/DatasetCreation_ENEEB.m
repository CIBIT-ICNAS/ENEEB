...
%% Adding information on probe placment Channel <-> Source Detector Link
TableOfLinks = table2cell(hb3(1).probe.link);

Channel = {};
for i = 1: size(TableOfLinks,1)
    Channel{i} = ['S',num2str(TableOfLinks{i,1}), 'D',num2str(TableOfLinks{i,2}),'-', TableOfLinks{i,3}];
end
